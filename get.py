import requests
import json
from departements import liste_departement

def is_departement(search):
    # Si c'est un département : True, sinon c'est une ville : False
    x = liste_departement()
    if search in x:
        return True
    return False
    
def get_specs_departement(departement):
...
def get_specs_city(ville):
    response = requests.get(f'https://data.economie.gouv.fr/api/explore/v2.1/catalog/datasets/prix-des-carburants-en-france-flux-instantane-v2/records?where=ville%3A"{ville.capitalize()}"&limit=-1')
    print(f'https://data.economie.gouv.fr/api/explore/v2.1/catalog/datasets/prix-des-carburants-en-france-flux-instantane-v2/records?where=ville%3A"{ville.capitalize()}"&limit=-1')
    data_dict = json.loads(response.text)
    return data_dict
...
def info_vraiment_importante(sous_liste,carburant):
    assert carburant in ["gazole", "sp95", "e85", "gplc", "e10", "sp98"],"Mauvais type de carburant"
    for element in sous_liste:
        if carburant in element:
            info_importante = element
    return [info_importante, [sous_liste[2], sous_liste[3]]]
...
if __name__=="__main__":
    ...
    # print(info_vraiment_importante(['latitude: 4925636.7262787', 'longitude: 613017.46660705', 'adresse: RUE CLEMENCEAU', 'ville: Amnéville', 'gazole_prix: 1.739', 'sp95_prix: 1.759', 'e85_prix: 0.985', 'gplc_prix: None', 'e10_prix: 1.729', 'sp98_prix: None'],"sp95"))
   
    # result = trouver_prix_moins_cher(supp_doublons(afficher_important(afficher_donnees(split_dico(get_specs_city("Thionville"))))), "sp95")
    # result = info_vraiment_importante(result,"sp95")
    # print(result)
    
    print(get_specs_city("Thionville"))
